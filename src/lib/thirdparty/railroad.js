...
export class Path extends FakeSVG {
  ...
  arc(sweep) {
    ...
    this.attrs.d +=
      "a" + Options.AR + " " + Options.AR + " 0 0 " + cw + " " + x + " " + y;
    return this;
  }
  arc_8(start, dir) {
    // 1/8 of a circle
...
export class OptionalSequence extends DiagramMultiContainer {
  ...
  toTextDiagram() {
    ...
    var SOILHeight = Math.max(
      ...itemTDs.slice(0, -1).map(function (itemTD) {
        return itemTD.entry;
      }),
    );
...
export class AlternatingSequence extends DiagramMultiContainer {
  ...
  toTextDiagram() {
    ...
    separator.push(
      line.repeat(leftWidth - 1) +
        corner_top_right +
        " " +
        corner_top_left +
        line.repeat(rightWidth - 2),
    );
    separator.push(
      " ".repeat(leftWidth - 1) +
        " " +
    ...
    diagramTD = diagramTD.alter(
      firstTD.height + Math.trunc(separator.length / 2),
      firstTD.height + Math.trunc(separator.length / 2),
    );
    var leftTD = new TextDiagram(
      firstTD.height + Math.trunc(separator.length / 2),
      firstTD.height + Math.trunc(separator.length / 2),
      leftLines,
    );
    var rightTD = new TextDiagram(
      firstTD.height + Math.trunc(separator.length / 2),
      firstTD.height + Math.trunc(separator.length / 2),
      rightLines,
    );
    diagramTD = leftTD.appendRight(diagramTD, "").appendRight(rightTD, "");
    diagramTD = new TextDiagram(1, 1, [
      corner_top_left,
      tee_left,
      corner_bot_left,
    ])
      ...
      .appendRight(
        new TextDiagram(1, 1, [corner_top_right, tee_right, corner_bot_right]),
        "",
      );
    return diagramTD;
  }
...
export class Choice extends DiagramMultiContainer {
  ...
  format(x, y, width) {
    ...
    for (var i = this.normal - 1; i >= 0; i--) {
      ...
      let lowerItem = this.items[i + 1];
      distanceFromY +=
        lowerItem.up + this.separators[i] + item.down + item.height;
      new Path(x, y)
        .arc("se")
    ...
    for (var i = this.normal + 1; i <= last; i++) {
      ...
      let upperItem = this.items[i - 1];
      distanceFromY +=
        upperItem.height + upperItem.down + this.separators[i - 1] + item.up;
      new Path(x, y)
        .arc("ne")
...
export class HorizontalChoice extends DiagramMultiContainer {
  ...
  toTextDiagram() {
    ...
    var baselineToSUIL = Math.max(
      ...itemTDs.slice(1).map(function (itemTD) {
        return itemTD.height - Math.min(itemTD.entry, itemTD.exit) - 1;
      }),
    );
    ...
    for (const [itemNum, itemTD] of enumerate(itemTDs)) {
      ...
      if (itemNum > 0) {
        ...
        for (
          i = 0;
          i < baselineToSUIL - (itemTD.height - itemTD.entry) + 1;
          i++
        ) {
          lines.push(" ".repeat(itemTD.width));
        }
      ...
      if (itemNum < itemTDs.length - 1) {
        ...
        for (
          i = 0;
          i < baselineToSUIL - (diagramTD.exit - diagramTD.entry);
          i++
        ) {
          lines.push(" " + line_vertical);
        }
...
export class Optional extends FakeSVG {
  constructor(item, skip) {
    if (skip === undefined) return new Choice(1, new Skip(), item);
    else if (skip === "skip") return new Choice(0, new Skip(), item);
...
export class OneOrMore extends FakeSVG {
  ...
  toTextDiagram() {
    ...
    for (
      var i = 0;
      i < itemTD.height - itemTD.entry + repeatTD.entry - 1;
      i++
    ) {
...
export class Start extends FakeSVG {
  ...
  format(x, y) {
    ...
    if (this.label) {
      new FakeSVG(
        "text",
        { x: x, y: y - 15, style: "text-anchor:start" },
        this.label,
      ).addTo(this);
    }
    return this;
  ...
  toTextDiagram() {
    var [cross, line, tee_right] = TextDiagram._getParts([
      "cross",
      "line",
      "tee_right",
    ]);
...
export class End extends FakeSVG {
  ...
  toTextDiagram() {
    var [cross, line, tee_left] = TextDiagram._getParts([
      "cross",
      "line",
      "tee_left",
    ]);
...
export class TextDiagram {
  constructor(entry, exit, lines) {
    ...
    if (entry > lines.length) {
      throw new Error(
        "Entry is not within diagram vertically:\n" + this._dump(false),
      );
    }
    if (exit > lines.length) {
      throw new Error(
        "Exit is not within diagram vertically:\n" + this._dump(false),
      );
    }
    for (var i = 0; i < lines.length; i++) {
      if (lines[0].length != lines[i].length) {
        throw new Error(
          "Diagram data is not rectangular:\n" + this._dump(false),
        );
      }
    }
  }
  ...
  appendRight(item, charsBetween) {
    ...
    var newHeight =
      Math.max(this.height - this.exit, item.height - item.entry) + joinLine;
  ...
  center(width, pad) {
    ...
    if (width === this.width) {
      ...
      return new TextDiagram(
        this.entry,
        this.exit,
        TextDiagram._encloseLines(this.lines, left, right),
      );
    }
  }
  copy() {
  ...
  expand(left, right, top, bottom) {
    ...
    if (left + right + top + bottom === 0) {
      ...
      for (i = 0; i < this.height; i++) {
        ...
        newLines.push(
          leftExpansion.repeat(left) +
            this.lines[i] +
            rightExpansion.repeat(right),
        );
      }
      for (i = 0; i < bottom; i++) {
        newLines.push(" ".repeat(this.width + left + right));
  ...
  _dump(show = true) {
    ...
    for (
      var y = 0;
      y < Math.max(this.lines.length, this.entry + 1, this.exit + 1);
      y++
    ) {
  ...
  static _padL(string, width, pad) {
    ...
    if ((width - string.length) % pad.length != 0) {
      throw new Error(
        "Gap " +
          (width - string.length) +
          " must be a multiple of pad string '" +
          pad +
          "'",
      );
    }
